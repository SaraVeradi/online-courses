...
def rock_spawner():
    ...
    #I want my rock to move in every possible direction,so I will chose a random diretion
    #Anothe possible solution is to chose a random direction from a list of all possible directions
    #, but this one here is my solution.
    ang_vel_direction = random.randrange(0,2)
    if ang_vel_direction == 0 :
        ang_vel_direction = -1
    rock_ang_vel = ang_vel_direction * random.randrange(MAX_ROTATION_VEL) / 60.0 
    rock_vel = [random.randrange(MAX_MOVEMENT_VEL) / 60.0 ,random.randrange(MAX_MOVEMENT_VEL) / 60.0]
    rock_vel_direction = random.randrange(0,2)
    if rock_vel_direction ==0 :
